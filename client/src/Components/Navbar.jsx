...
const Navbar = () => {
  ...
  const navigate = useNavigate();   // ⭐ FIXED (use router navigate)
  ...
  const { user, isOwner, setShowHotelReg } = useAppContext();  // ⭐ removed navigate from context
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${
      ...
      <div className="hidden md:flex items-center gap-4">
        ...
        <img src={assets.searchIcon} alt="search" className={`${isScrolled && "invert"} h-7`} />
      ...
      {/* MOBILE MENU */}
      <div className={`fixed top-0 left-0 w-full h-screen bg-white flex flex-col md:hidden items-center justify-center gap-6 transition-all ${
